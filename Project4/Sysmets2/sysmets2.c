...
{
	// 静态无链接变量
	static int cxChar; // 字体平均宽度
	static int cxCaps;
	static int cyChar; // 字体平均高度
	static int cyClient; // 窗口垂直高度
	...
	switch (message)
	{
		...
		case WM_SIZE:
			...
			// 设置滚动条垂直滚动范围，位置
			maxPos = max(0, NUMLINES - cyClient / cyChar);
		...
		case WM_VSCROLL:
			...
			iVscrollPos = max(0, min(iVscrollPos, maxPos));
